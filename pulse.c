...
static int pulse_request_gtc(struct openconnect_info *vpninfo, struct oc_text_buf *reqbuf,
			     uint8_t eap_ident, char *gtc_prompt)
{
	struct oc_auth_form f;
	struct oc_form_opt o;
	int ret;
	...
	o.name = (char *)"tokencode";
	o.label = gtc_prompt;

	if (!can_gen_tokencode(vpninfo, &f, &o))
		o.type = OC_FORM_OPT_TOKEN;
	...
	if (o._value) {
		...
		buf_append_bytes(reqbuf, eap_avp, sizeof(eap_avp));
		buf_append_bytes(reqbuf, o._value, l);
